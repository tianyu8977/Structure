#include<memory>
#include<cmath>
#include"Refine_Block2D_I.h"
#include"../Curve2D/Spline2D.h"


knoblauch::Block2D_Refined_I knoblauch::Refine_Block2D_In_I(const Block_2D& block) {
  
  Block2D_Refined_I new_blocks;
  ...
  int new_number_nodes = num_nodes_i*num_nodes_j+(num_nodes_i-1)*num_nodes_j;
  int new_number_i_neg = 0;
  int new_number_i_pos = 0;
  int current_number = 0;
  float_type s = 0.0;

  std::vector<Vector2D> new_nodes_all(new_number_nodes);
  std::vector<Vector2D> new_nodes_i_pos(number_of_nodes);
  std::vector<Vector2D> new_nodes_i_neg(number_of_nodes);
  std::vector<Vector2D> boundary_nodes(num_nodes_j);
  ...
  for(int i=0; i<num_nodes_i-1; i++) {
    current_number = (2*num_nodes_i-1)*(num_nodes_j-1)+2*i+1;
  ...
  for(int j=0; j<num_nodes_j; j++) {
    for(int i=0; i<num_nodes_i;i++) {
      new_nodes_i_neg[new_number_i_neg] = new_nodes_all[current_number];
      new_number_i_neg++;
      current_number++;
    }  // for i
  ...
  for(int j=0; j<num_nodes_j; j++) {
    for(int i=0; i<num_nodes_i;i++) {
      new_nodes_i_pos[new_number_i_pos] = new_nodes_all[current_number];
      new_number_i_pos++;
      current_number++;
    }  // for i
    current_number = current_number+num_nodes_i-1;
  ...
  for(int k=0; k<num_nodes_j; k++) {
    int boundary_number = num_nodes_i*k+num_nodes_i-1;
    boundary_nodes[k] = new_nodes_i_neg[boundary_number];
  }

  std::shared_ptr<Curve2D_Base> boundary_i_new( new Spline2D(boundary_nodes) );
  //////////////////////////////  build boundary i_new

  new_blocks.i_neg = std::unique_ptr<Block_2D>(new Block_2D(num_nodes_i,
							    num_nodes_j,
							    new_nodes_i_neg, 
							    boundary_i_new,
  ...
  //////////////////////////////////////////////////////  done new block of i neg



  new_blocks.i_pos = std::unique_ptr<Block_2D>(new Block_2D(num_nodes_i,
							    num_nodes_j,
							    new_nodes_i_pos,
